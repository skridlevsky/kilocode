...
describe("Fzf - Word Boundary Matching", () => {
	describe("Basic word boundary matching", () => {
		...
		it("should match at text start with highest priority", () => {
			const items = [
				{ id: 1, name: "foo bar" },
				{ id: 2, name: "the foo" },
			]
			...
			// "foo bar" should score higher than "the foo"
	...
	describe("Scoring and ranking", () => {
		it("should rank exact matches highest", () => {
			...
			// "test" should be first (exact match at start)
			expect(results[0].item.id).toBe(1)
		})

		it("should rank start matches higher than middle matches", () => {
			const items = [
				{ id: 1, name: "foo bar" },
				{ id: 2, name: "bar foo" },
			]
			...
			// "foo bar" should rank higher than "bar foo"
			expect(results[0].item.id).toBe(1)
		})

		it("should rank longer query matches higher", () => {
			const items = [
				{ id: 1, name: "foobar" },
				{ id: 2, name: "foo" },
			]
	...
	describe("Substring matching within words", () => {
		it("should match substrings within words with lower priority", () => {
			const items = [
				{ id: 1, name: "foo bar" },
				{ id: 2, name: "barfoo" },
			]
			...
			// Both should match, but "foo bar" should rank higher
			expect(results).toHaveLength(2)
			expect(results[0].item.id).toBe(1)
			expect(results[1].item.id).toBe(2)
		})
	...
	describe("Acronym matching", () => {
		it("should match acronyms from word starts", () => {
			...
			expect(results.length).toBeGreaterThan(0)
			expect(results[0].item.name).toBe("Claude Sonnet")
		})
		...
		it("should prefer direct matches over acronym matches", () => {
			const items = [{ name: "clso tool" }, { name: "Claude Sonnet" }]
			const fzf = new Fzf(items, { selector: (item) => item.name })

			const results = fzf.find("clso")
			// Direct match "clso tool" should rank higher than acronym "Claude Sonnet"
	...
	describe("Edge cases", () => {
		...
		it("should handle special characters in query", () => {
			...
			// Should match "foo-bar" as "foo-" is at the start
			expect(results).toHaveLength(1)
		})
	})
